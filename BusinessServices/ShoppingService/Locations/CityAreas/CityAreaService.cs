...
namespace FMASolutionsCore.BusinessServices.ShoppingService
{
    public class CityAreaService : ICityAreaService
    {
        ...
        public bool CreateNew(CityArea model)
        {
            try
            {
                ...
                if (ValidateForCreate(model))
                {
                    ...
                    if (success)
                    {
                        CityArea createdModel = GetByCode(model.CityAreaCode);
                        model.CityAreaID = createdModel.CityAreaID;
                        _uow.SaveChanges();
                    }
                    else
        ...
        private bool ValidateForCreate(CityArea model)
        {
            ...
            {
                if (string.IsNullOrEmpty(model.CityAreaCode) || string.IsNullOrEmpty(model.CityAreaName) ||model.CityID <= 0)
                {
                    model.ModelState.AddError("NullValues", "All values must be populated...");
                    return false;
                }
                else if (model.CityAreaCode.Length > 7)
                {
